...
<html ng-app="demoapp">
   ...
   <body  ng-controller="MarkersAngularTemplateController">
      ...
      <div   class="page-footer" style="width:100%;height:50%, background-color:grey" id="solicitado" ng-show="marker.status == 1">Solicitado : {{marker.cliente}} - <img ng-src="{{marker.foto}}">  <br><a ng-href="geo:{{marker.location}}" class="button btn">ir waze</a><a ng-href="https://maps.apple.com/maps?q={{marker.location}}&navigate=yes" class="button btn">ir maps</a><br>
         <a ng-if="embarcado != 1" ng-click="embarcar(marker.cliente_uid, marker.cliente)" class="button btn">Embarcou</a>
         <a ng-if="embarcado == 1" ng-click="finalizar_corrida(marker.cliente_uid, marker.cliente)" class="button btn">Finalizar Corrida</a>
         <a ng-if="finalizado == 1" ng-click="receber(marker.cliente_uid, marker.cliente)" class="button btn">receber do cliente</a>
      </div>
      <div class="fixed-action-btn vertical click-to-toggle" id="menu_locais">
         ...
         <ul>
         <ul>
            <li ng-hide="habilitado"><a class="btn-floating green darken-1" ng-click="habilitar()"><i class="material-icons">directions_car</i></a></li>
            <li ng-show="habilitado"><a class="btn-floating yellow darken-1" ng-click="desabilitar()"><i class="material-icons">directions_car</i></a></li>
      ...
      //-23,5486. Longitude de Praça da Sé é -46,6392
             ...
             $scope.habilitado = false;
                    ...
                    // $scope.markers.m1.lat = $geolocation.position.coords.latitude;
                     ...
                     $scope.embarcar = function(cliente_uid , nome){
                         alert("Embarcou cliente "+nome);
                         firebase.database().ref('/riders/' + cliente_uid).update({ status: "4"});
                         $scope.embarcado = 1;
                       } 

                       $scope.finalizar_corrida = function(cliente_uid , nome){
                         alert("Vai fazer o calculo da corrida [ainda deve ser feito] ");
                         firebase.database().ref('/riders/' + cliente_uid).update({ status: "6"});
                         $scope.embarcado = 0;
                         $scope.finalizado = 1;
                       } 

                       $scope.receber = function(cliente_uid , nome){
                         var retVal = confirm("Recebeu no local ?");
                         if( retVal == true ){
                            //document.write ("User wants to continue!");
                            var tipo_pagamento = 9;
                         }
                         else{
                            //document.write ("User does not want to continue!");
                            var tipo_pagamento = 8;
                         }
                         firebase.database().ref('/riders/' + cliente_uid).update({ status: tipo_pagamento});
                         $scope.embarcado = 0;
                         $scope.finalizado = 0;
                       } 
